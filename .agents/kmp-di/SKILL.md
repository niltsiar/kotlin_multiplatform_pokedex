...
Before adding Koin definitions, ask yourself:

1. **Where does this belong?**
   - Shared business logic (repos, ViewModels) → `:features:<feature>:wiring/commonMain`
   - Platform-specific navigation → `:features:<feature>:wiring/androidMain` or `jvmMain`
   - Core infrastructure (Navigator, HttpClient) → `:core:di`
   - NEVER in `:core` for feature-specific code

2. **What scope is appropriate?**
   - Stateful/expensive resources (HttpClient, Database) → `single`
   - Stateless services (Repositories, ViewModels) → `factory`
   - ViewModels → ALWAYS use `viewModel` DSL, never `single`

3. **Does it need parameters?**
   - Runtime values (IDs from navigation) → Use `params` lambda
   - Static config (baseUrl, apiKey) → Use named qualifiers
   - DI-resolved dependencies → Use `get()`
...
**For detailed Koin reference (setup, patterns, testing, troubleshooting)**: See [Official Koin Documentation](https://insert-koin.io)
